...
// })
// bot.use(adminStage.middleware())

const stage = new Stage([gsWizard, setAdminWizard])
// User services stage
gsWizard.enter(async ctx => {
  await ctx.reply('Будь-ласка, вкажіть ПІБ людини')
})

stage.hears('Вийти', async ctx => {
  if (currentUser.role == 'User') {
    await ctx.reply('Оберіть послугу в Меню', remove_kb)
    return ctx.scene.leave()
  } else if (currentUser.role == ('Admin' || 'Super_Admin')) {
    await ctx.reply('Оберіть команду в меню', remove_kb)
    return ctx.scene.leave()
  }
})

stage.hears('Підтвердити', async ctx => {
  await ctx.reply(
  ...
  return new Promise((resolve, reject) => {
    base(tableUsers)
      ...
      .firstPage(async function (err, records) {
        ...
        if (err) {
          console.error('problem when retrieve data')
          reject(err)
        }
        if (records.length > 0) {
...
if (process.env.NODE_ENV == 'development') {
  ...
  bot.launch().then(() => {
    console.log(`The bot ${bot.botInfo.username} is running locally`)
  })
} else {
  // Launch on webhook
  bot
    ...
    .then(() => {
      console.log(`The bot ${bot.botInfo.username} is running on server`)
    })
}
